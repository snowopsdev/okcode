...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@okcode/contracts",
    "--filter=@okcode/web",
    "--filter=okcode",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=okcode"],
  "dev:web": ["run", "dev", "--filter=@okcode/web"],
  "dev:desktop": ["run", "dev", "--filter=@okcode/desktop", "--filter=@okcode/web", "--parallel"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("OKCODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("OKCODE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid OKCODE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `OKCODE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric OKCODE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed OKCODE_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_OKCODE_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly okcodeHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(okcodeHome);

    const output: NodeJS.ProcessEnv = {
      ...baseEnv,
      OKCODE_PORT: String(serverPort),
      PORT: String(webPort),
      ELECTRON_RENDERER_PORT: String(webPort),
      VITE_WS_URL: `ws://localhost:${serverPort}`,
      VITE_DEV_SERVER_URL: devUrl?.toString() ?? `http://localhost:${webPort}`,
      OKCODE_HOME: resolvedBaseDir,
    };

    if (host !== undefined) {
      output.OKCODE_HOST = host;
    }

    if (authToken !== undefined) {
      output.OKCODE_AUTH_TOKEN = authToken;
    } else {
      delete output.OKCODE_AUTH_TOKEN;
    }

    if (noBrowser !== undefined) {
      output.OKCODE_NO_BROWSER = noBrowser ? "1" : "0";
    } else {
      delete output.OKCODE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.OKCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.OKCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.OKCODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.OKCODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.OKCODE_MODE = "web";
      delete output.OKCODE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read OKCODE_PORT_OFFSET/OKCODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("OKCODE_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv("OKCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD"),
      logWebSocketEvents: readOptionalBooleanEnv("OKCODE_LOG_WS_EVENTS"),
    };
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.OKCODE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.OKCODE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  okcodeHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all OK Code data (equivalent to OKCODE_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("OKCODE_HOME")),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to OKCODE_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("OKCODE_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to OKCODE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("OKCODE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to OKCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("OKCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to OKCODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("OKCODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to OKCODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("OKCODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to OKCODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("OKCODE_PORT")),
  ),
