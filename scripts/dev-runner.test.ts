...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit OKCODE_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "OKCODE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid OKCODE_PORT_OFFSET"));
      }),
    );
  });
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("defaults OKCODE_HOME to ~/.t3 when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.OKCODE_HOME, resolve(homedir(), ".okcode"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.OKCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.OKCODE_LOG_WS_EVENTS, "1");
        assert.equal(env.OKCODE_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            OKCODE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          okcodeHome: undefined,
        ...
        assert.equal(env.OKCODE_MODE, "web");
        assert.equal(env.OKCODE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.OKCODE_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom okcodeHome when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.OKCODE_HOME, resolve("/tmp/my-t3"));
      }),
    );
  });
