...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { okcodeCommitHash?: unknown; t3codeCommitHash?: unknown };
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.OKCODE_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("OK Code failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const disabledReason = getAutoUpdateDisabledReason({
    ...
    disabledByEnv: process.env.OKCODE_DISABLE_AUTO_UPDATE === "1",
  });
  if (disabledReason) {
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `OK Code ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
...
function resolveUserDataPath(): string {
  ...
  const legacyElectronPath = Path.join(appDataBase, LEGACY_USER_DATA_DIR_NAME);
  if (FS.existsSync(legacyElectronPath)) {
    return legacyElectronPath;
  }

  const legacyT3codePath = Path.join(appDataBase, "t3code");
  if (FS.existsSync(legacyT3codePath)) {
    return legacyT3codePath;
  }
...
function configureAutoUpdater(): void {
  ...
  const githubToken =
    process.env.OKCODE_DESKTOP_UPDATE_GITHUB_TOKEN?.trim() || process.env.GH_TOKEN?.trim() || "";
  if (githubToken) {
...
function backendEnv(): NodeJS.ProcessEnv {
  return {
    ...
    OKCODE_MODE: "desktop",
    OKCODE_NO_BROWSER: "1",
    OKCODE_PORT: String(backendPort),
    OKCODE_HOME: BASE_DIR,
    OKCODE_AUTH_TOKEN: backendAuthToken,
  };
}
...
async function bootstrap(): Promise<void> {
  ...
  process.env.OKCODE_DESKTOP_WS_URL = backendWsUrl;
