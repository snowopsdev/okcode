...
describe("TerminalManager", () => {
  ...
  it("filters app runtime env variables from terminal sessions", async () => {
    ...
    try {
      ...
      expect(spawnInput.env.OKCODE_PORT).toBeUndefined();
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    expect(spawnInput.env.OKCODE_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.OKCODE_WORKTREE_PATH).toBe("/repo/worktree-a");
